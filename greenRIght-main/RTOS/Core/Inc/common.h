...
typedef struct {
    ...
    uint8_t turn_signal;     /* 2 bit - 2차에서 추가 */
} EgoVehicle;
...
typedef struct {
	...
	uint16_t heading;        /* 2차 CAN Protocol */
...
#define DECISION_DATA_STATUS_STALE             (0x02U)
#define DECISION_DATA_STATUS_TIME_SYNC_ERROR   (0x04U)
